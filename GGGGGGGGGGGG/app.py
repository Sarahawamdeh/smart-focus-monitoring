...
def last_report():
    ...
    ''', (sess_id,)).fetchone()
    conn.close()
    return render_template('last_p.html', report=report_data)
# صفحة التقرير
...
def lecture_details():
    ...
    if report_data:
        # تأكدي أن اسم الملف هنا يطابق ملف الـ HTML (lecture-details.html)
        return render_template('lecture-details.html', report=report_data)
...
def save_session():
    ...
    # إذا الـ JavaScript ما بعث الـ ID، رح نعطيه رقم 1 كاحتياط
    ...
    try:
        ...
        # استخدام [session] بالأقواس المربعة مهم جداً في SQLite
        cursor.execute('''
        ...
        print(f"✅ تم الحفظ بنجاح! ID الجلسة الجديد هو: {new_id}")
